...
export default function StockSearch({ stocks }) {
  ...
  const formatRating = (val) => {
    if (val == null) return "-";
    let label;
    if (val <= 1.5) label = t("rec.strongBuy");
    else if (val <= 2.5) label = t("rec.buy");
    else if (val <= 3.5) label = t("rec.hold");
    else if (val <= 4.5) label = t("rec.sell");
    else label = t("rec.strongSell");
      ...
      {selected && (
        <div className="absolute top-full right-0 mt-1 w-72 sm:w-96 bg-white border border-gray-200 rounded-lg shadow-lg z-50 p-3 sm:p-4">
          <div className="flex justify-between items-start mb-3">
            <div>
              <h3 className="text-lg font-bold text-gray-900">{selected.symbol}</h3>
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
            <div className="flex justify-between">
              <span className="text-gray-500 w-20">{t("search.rsi")}</span>
            ...
            <div className="flex justify-between">
              <span className="text-gray-500 w-20">{t("search.revGrowth")}</span>
            ...
            <div className="flex justify-between">
              <span className="text-gray-500 w-20">{t("search.analyst")}</span>
              <span className="font-mono text-gray-900 text-right flex-1 text-xs">{formatRating(selected.recommendation_mean)}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-500 w-20">{t("search.divYield")}</span>
